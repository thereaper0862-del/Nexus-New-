...
(() => {
  ...
  function openPlay(id) {
    ...
    if (embed) {
      frame.innerHTML = `
        ...
        <iframe src="${embed}" loading="lazy" allowfullscreen allow="autoplay; fullscreen; gamepad; pointer-lock; clipboard-write; encrypted-media; picture-in-picture" sandbox="allow-scripts allow-same-origin allow-forms allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-fullscreen"></iframe>`;
    } else if (url) {
  ...
  $('#modalFull').addEventListener('click', () => {
    ...
    if (!target) return;
    if (playModal.classList.contains('fs')) { exitImmersive(); return; }
    if (document.fullscreenElement) { document.exitFullscreen().catch(() => {}); return; }
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) ||
      (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    const canNative = typeof document.documentElement.requestFullscreen === 'function' && !isIOS;
    if (canNative) {
      target.requestFullscreen().catch(() => enterImmersive());
    } else {
      enterImmersive();
    }
  });
  function enterImmersive() {
    playModal.classList.add('fs');
    document.body.style.overflow = 'hidden';
    toast('Immersive mode on — tap the fullscreen button to exit. 📱');
  }
  function exitImmersive() {
    playModal.classList.remove('fs');
    document.body.style.overflow = '';
    toast('Immersive mode off');
  }
  document.addEventListener('fullscreenchange', () => {
    if (!document.fullscreenElement) playModal.classList.remove('fs');
  });
